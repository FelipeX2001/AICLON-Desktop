...
const ActiveClientsBoard: React.FC<ActiveClientsBoardProps> = ({ user, users }) => {
  ...
  const togglePayment = (e: React.MouseEvent, id: string) => {
    ...
    setActiveClients(prev => prev.map(c => 
        c.activeId === id ? { ...c, pago_mes_actual: !c.pago_mes_actual } : c
    ));
  };

  const handleSaveClient = (updatedClient: ActiveClient) => {
      setActiveClients(prev => prev.map(c => c.activeId === updatedClient.activeId ? updatedClient : c));
  ...
  const handleDrop = (e: React.DragEvent, stage: ActiveClientStage) => {
    ...
    if (client && client.estado_servicio !== stage) {
      setActiveClients(prev => prev.map(c => 
        c.activeId === id ? { ...c, estado_servicio: stage } : c
      ));
    }
  };
      ...
      >
        <div className="flex h-full space-x-4 min-w-max px-1">
          {ACTIVE_CLIENT_STAGES.map((stage) => {
            const stageClients = activeClients.filter(c => c.estado_servicio === stage);
